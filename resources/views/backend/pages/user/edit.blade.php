<form id="editUserForm" action="{{ route('admin.user.update', $user->id) }}" method="post">
    ...
    <div class="modal-body">
        ...
        <div class="form-group  row">
            ...
            <div class="col-sm-9">
                <input type="file" onchange="previewFile(this, 'previewImg')" class="form-control"
                    name="profile_image" id="profile_image">
                <img id="previewImg"
                    src="{{ $user->profile_image ? asset($user->profile_image) : asset('backend/images/no-img.jpg') }}"
            ...
            <div class="col-sm-10">
                <select name="role" class="form-control" required>
                    @foreach (App\Models\Role::all() as $item)
                        @if ($item->id > 1)
                            <option @if ($user->role == $item->id) selected @endif value="{{ $item->id }}">
                                {{ $item->name }}</option>
                        @else
        ...
        <div class="form-group  row">
            ...
            <div class="col-sm-3 d-flex align-items-center">
                <div class="form-check form-switch">
                    <input class="form-check-input" @if ($user->status == 1) checked @endif type="checkbox"
